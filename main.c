...
void shift_values(uint8_t step);
...
void generate_colors() {
	uint8_t red = 255, blue = 0, green = 0;
	uint8_t stage = LED_COUNT / 3, devider = 255 / stage;
...
void shift_values(uint8_t step) {
	...
	for (uint8_t i = LED_COUNT - step, j = 0; i < LED_COUNT; i++, j++) {
		buf[j] = colors[i];
	}	
	
	for (uint8_t i = LED_COUNT - step - 1, index = LED_COUNT - 1; i > 0; i--, index--) {
